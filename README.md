...
- `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` - La clave pública (Publishable API key)

Las credenciales se encuentran en tu [dashboard de Supabase](https://supabase.com/dashboard/project/_/settings/api).

### Proveedores de autenticación

En el dashboard de Supabase, habilita los siguientes proveedores en **Authentication > Providers**:

- **Google**: Configura con tu OAuth Client ID y Secret de Google Cloud Console
- **Facebook**: Configura con tu App ID y Secret de Facebook Developers
- **Email**: Habilitado por defecto

Asegúrate de configurar las URLs de redirección correctas para cada proveedor.

## Estado actual

- Página de bienvenida con opciones de registro (Google, Facebook, correo electrónico).
...
- Próximos pasos incluyen carga/visualización de actas y sistema de verificación.

## Desarrollo local

1. Clona el repositorio y prepara dependencias con `pnpm install` (o `npm install` si prefieres).
2. Copia `.env.local.example` a `.env.local` y agrega tus credenciales de Supabase.
3. Levanta el servidor de desarrollo con `pnpm dev` y abre `http://localhost:3000`.
4. Otros scripts útiles: `pnpm build` para compilar, `pnpm start` para producción y `pnpm lint` para validar el código.
